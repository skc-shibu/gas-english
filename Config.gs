...
// ──────────────────────────────────────────────
var LOG_HEADERS = ['timestamp', 'wordCount', 'idiomCount', 'prompt', 'response', 'error'];
...
function validateHeaders(sheet, expectedHeaders) {
  ...
  for (var i = 0; i < expectedHeaders.length; i++) {
    ...
    if (actual !== expected) {
      throw new Error(
        '「' + sheetName + '」シートのヘッダが不正です。' +
        '列' + (i + 1) + ': 期待="' + expected + '", 実際="' + actual + '"'
      );
    }
  }
}
